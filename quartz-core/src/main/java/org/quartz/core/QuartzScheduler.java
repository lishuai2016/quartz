...
public class QuartzScheduler implements RemotableQuartzScheduler {
    ...
    private QuartzSchedulerThread schedThread;//调度器线程
    ...
    public QuartzScheduler(QuartzSchedulerResources resources, long idleWaitTime, @Deprecated long dbRetryInterval)
        ...
        this.schedThread = new QuartzSchedulerThread(this, resources);//创建一个调度器线程
        ThreadExecutor schedThreadExecutor = resources.getThreadExecutor();
        schedThreadExecutor.execute(this.schedThread);//启动带哦读取线程
        if (idleWaitTime > 0) {
     ...
     * </p>
     */
    public void start() throws SchedulerException {//启动调度器
        ...
        //作用是使调度器线程跳出一个无限循环，开始轮询所有trigger触发job
    ...
    public Date scheduleJob(JobDetail jobDetail,
        ...
        notifySchedulerListenersJobAdded(jobDetail);// 通知job监听者
        notifySchedulerThread(trigger.getNextFireTime().getTime());// 通知调度器线程
        notifySchedulerListenersSchduled(trigger);// 通知trigger监听者
...
J     *
    ...
    protected void notifySchedulerThread(long candidateNewNextFireTime) {//增删改trigger都会触发
