...
public interface Trigger extends Serializable, Cloneable, Comparable<Trigger> {
     ...
     * by key.
     */
    class TriggerTimeComparator implements Comparator<Trigger>, Serializable {//触发时机比较器
        ...
        public static int compare(Date nextFireTime1, int priority1, TriggerKey key1, Date nextFireTime2, int priority2, TriggerKey key2) {
            if (nextFireTime1 != null || nextFireTime2 != null) {//1、先比较触发时间
            ...
            int comp = priority2 - priority1;//2、比较优先级
            if (comp != 0) {
                return comp;
            }

            return key1.compareTo(key2);//3、按照名字排序
        }
